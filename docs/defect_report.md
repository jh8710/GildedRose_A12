...
| 문서 | 역할 |
| --- | --- |
| [requirements_analysis.md](requirements_analysis.md) | 아이템 유형별 비즈니스 규칙·경계값·시나리오 |
| [test_plan.md](test_plan.md) | 테스트 우선순위(P0~P2), 경계값 매트릭스, JaCoCo 목표 |
...
4. `mvn test` Green이나 요구사항 매핑·커버리지 갭만 있으면 **Info**로 기록한다(현재 [06_테스트_실행_결함_분석_보고서](../report/06_테스트_실행_결함_분석_보고서.md) 상태).
...
스프린트 보고 시 **단계별 누적 막대**와 **Severity 분포**를 함께 제시한다.
...
`test_plan.md` §7과 동일 전략: `prepare-agent` → `report` → `check` 게이트.
...
주간(또는 PR마다) 아래 표를 `defect_list.md` 상단 또는 Wiki에 갱신한다.
...
| --- | --- |
| `defect` | 기능 결함 (기본) |
| `severity:critical` / `major` / `minor` / `info` | §2.1 Severity |
| `item:normal` / `aged-brie` / `backstage` / `sulfuras` / `conjured` | §2.2 ItemType |
| `phase:T1-unit` … `phase:T5-ci` | 발견 단계 |
| `status:verified` | 수정·테스트 완료, Close 전 |

### 5.2 이슈 본문 ↔ 보고서 매핑

GitHub Issue 설명에 §3 템플릿 섹션을 그대로 사용한다. 제목 형식:

```text
[DEF-CJ-MAJ-01][Conjured] 만료 전 quality 2 감소 미적용
```
...
| 이벤트 | GitHub 동작 | 결함 문서 |
| --- | --- | --- |
| Surefire 실패 on PR | CI annotation → (선택) bot이 `defect` 이슈 초안 | `defect_list.md` Open |
| PR 머지 | 연결 Issue `Fixes #N`로 자동 Close | 상태 Verified |
| Release | Open Critical/Major = 0 게이트 | 메트릭 표 스냅샷 |

### 5.4 Issue 템플릿 (`.github/ISSUE_TEMPLATE/defect.yml` 요약)
...
- PR 설명에 `Closes #123` 또는 `Fixes #123` 포함.
- Critical/Major는 **최소 1개** 실패 재현 테스트(또는 Golden Master 갱신)가 PR에 포함되어야 머지 가능.
- JaCoCo `check` 실패 시 PR에 `phase:T5-ci` 라벨 부여.
